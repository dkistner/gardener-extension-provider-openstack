...
type AddOptions struct {
	...
	// ApplicationCredentialConfig contains config for the usage of managed application credentials.
	ApplicationCredentialConfig controllerconfig.ApplicationCrendentialConfig
}
...
func AddToManagerWithOptions(mgr manager.Manager, options AddOptions) error {
	return infrastructure.Add(mgr, infrastructure.AddArgs{
		Actuator:          NewActuator(&options.ApplicationCredentialConfig),
		ConfigValidator:   NewConfigValidator(openstackclient.FactoryFactoryFunc(openstackclient.NewOpenstackClientFromCredentials), &options.ApplicationCredentialConfig, log.Log),
