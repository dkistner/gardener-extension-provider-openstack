...
func getConfigChartValues(
	...
	var cloudProfileLbClasses = []api.LoadBalancerClass{}
	if floatingPool, err := helper.FindFloatingPool(cloudProfileConfig.Constraints.FloatingPools, infraStatus.Networks.FloatingPool.Name, cp.Spec.Region, nil); err == nil {
		cloudProfileLbClasses = floatingPool.LoadBalancerClasses
	}

	var loadBalancerClasses = cloudProfileLbClasses
...
func lookupLoadBalancerClass(lbClasses []api.LoadBalancerClass, lbClassPurpose string) *api.LoadBalancerClass {
	...
	// If a lookup of the default LoadBalancerClass was requested and none was found
	// then the first configured LoadBalancerClass will be treated as default.
